...
-- Test deparsing of a PLAN clause on a multi-level chain of nested paths;
-- the child plan of OUTER/INNER must be parenthesized if it is not a bare
-- path name, or the deparsed definition would not re-parse (pg_dump/restore
-- would then fail).
CREATE VIEW jsonb_table_view_plan_chain AS
SELECT * FROM JSON_TABLE(
	jsonb '[{"a": 1, "b": [{"c": 2, "d": [3, 4]}]}]', '$[*]' AS p0
	COLUMNS (
		a int PATH '$.a',
		NESTED PATH '$.b[*]' AS p1 COLUMNS (
			c int PATH '$.c',
			NESTED PATH '$.d[*]' AS p11 COLUMNS (
				d int PATH '$'
			)
		)
	)
...
\sv jsonb_table_view_plan_chain
SELECT * FROM jsonb_table_view_plan_chain;
DROP VIEW jsonb_table_view_plan_chain;
