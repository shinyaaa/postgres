...
/*
 * pg_estimate_vacuum_wal
 *
 * Estimate WAL output bytes that would be generated by vacuuming the given
 * relation.  This is a read-only estimation based on table statistics and
 * visibility map information; it does not modify the relation.
 *
 * The estimation covers lazy vacuum only (not VACUUM FULL).
 *
 * Returns a record with the following fields:
 *   wal_bytes        - estimated total WAL bytes
 *   wal_fpi          - estimated number of full page images
 *   wal_fpi_bytes    - estimated FPI bytes
 *   heap_wal_bytes   - estimated heap-related WAL bytes (excluding FPI)
 *   index_wal_bytes  - estimated index-related WAL bytes
 *   dead_tuples      - number of dead tuples from statistics
 *   pages_to_scan    - number of heap pages to be scanned
 *   pages_all_visible - number of all-visible pages
 *   pages_all_frozen  - number of all-frozen pages
 */
Datum
pg_estimate_vacuum_wal(PG_FUNCTION_ARGS)
{
	...
	BlockNumber	dirty_pages;
	int64		n_dead_tup;
	int64		est_wal_fpi;
	int64		est_wal_fpi_bytes;
	int64		est_heap_wal_bytes;
	int64		est_index_wal_bytes;
	int64		est_wal_bytes;
	double		avg_dead_per_page;
	int			nindexes;
	Relation   *indrels;
	PgStat_StatTabEntry *tabentry;

	/* Only valid for ordinary relations */
	...
	rel_pages = RelationGetNumberOfBlocks(rel);

	/* Get visibility map counts */
	all_visible = 0;
	all_frozen = 0;
	visibilitymap_count(rel, &all_visible, &all_frozen);

	/* Get dead tuple count from statistics */
	n_dead_tup = 0;
	tabentry = pgstat_fetch_stat_tabentry(relid);
	if (tabentry != NULL)
		n_dead_tup = tabentry->dead_tuples;

	/*
	 * Classify pages.  Pages that are not all-visible will be scanned and
	 * may generate WAL for pruning/freezing.  All-visible pages are skipped
	 * in a normal (non-aggressive) vacuum.
	 */
	dirty_pages = (rel_pages > all_visible) ? (rel_pages - all_visible) : 0;

	/*
	 * Estimate heap WAL.
	 *
	 * Each dirty page with dead tuples generates one XLOG_HEAP2_PRUNE WAL
	 * record containing: XLogRecord header, block header, xl_heap_prune
	 * header, conflict horizon, and per-tuple offset data.
	 */
	est_heap_wal_bytes = 0;
	if (dirty_pages > 0 && n_dead_tup > 0)
	{
		int64	per_page_base;
		int64	per_page_tuples;

		avg_dead_per_page = (double) n_dead_tup / (double) dirty_pages;

		/* Base overhead per WAL record */
		per_page_base = SizeOfXLogRecord +
			SizeOfXLogRecordBlockHeader +
			SizeOfHeapPrune +
			sizeof(TransactionId);	/* conflict horizon */

		/*
		 * Per dead tuple: OffsetNumber for dead items + OffsetNumber for
		 * now-unused items
		 */
		per_page_tuples = (int64) (avg_dead_per_page *
								   (2 * sizeof(OffsetNumber)));

		est_heap_wal_bytes = dirty_pages * (per_page_base + per_page_tuples);
	}

	/*
	 * Estimate FPI (Full Page Images).
	 *
	 * A full page image is written the first time a page is modified after a
	 * checkpoint.  We use a heuristic of 50% of modified pages generating
	 * FPIs, as the actual rate depends on checkpoint timing relative to
	 * vacuum execution.
	 */
	est_wal_fpi = (int64) (dirty_pages * 0.5);
	est_wal_fpi_bytes = est_wal_fpi * BLCKSZ;

	/*
	 * Estimate index WAL.
	 *
	 * For each index, estimate the number of pages that will be touched
	 * during index vacuuming based on the dead tuple count and index density.
	 * This is a rough heuristic assuming btree-like behavior.
	 */
	est_index_wal_bytes = 0;
	vac_open_indexes(rel, AccessShareLock, &nindexes, &indrels);

	for (int i = 0; i < nindexes; i++)
	{
		Relation	indrel = indrels[i];
		BlockNumber	ind_pages;
		double		ind_tuples;
		double		tuples_per_page;
		int64		pages_touched;

		ind_pages = RelationGetNumberOfBlocks(indrel);
		ind_tuples = indrel->rd_rel->reltuples;

		if (ind_pages == 0 || ind_tuples <= 0 || n_dead_tup <= 0)
			continue;

		tuples_per_page = ind_tuples / (double) ind_pages;
		if (tuples_per_page <= 0)
			continue;

		/* Estimate index pages that will be modified */
		pages_touched = (int64) ceil((double) n_dead_tup / tuples_per_page);
		if (pages_touched > ind_pages)
			pages_touched = ind_pages;

		/*
		 * Each touched index page generates a WAL record.  Use 64 bytes as
		 * a heuristic average size for a btree deletion WAL record payload.
		 */
		est_index_wal_bytes += pages_touched *
			(SizeOfXLogRecord + SizeOfXLogRecordBlockHeader + 64);
	}

	vac_close_indexes(nindexes, indrels, AccessShareLock);

	/* Compute totals */
	est_wal_bytes = est_heap_wal_bytes + est_wal_fpi_bytes + est_index_wal_bytes;

	/* Build result tuple */
	values[0] = Int64GetDatum(est_wal_bytes);
	values[1] = Int64GetDatum(est_wal_fpi);
	values[2] = Int64GetDatum(est_wal_fpi_bytes);
	values[3] = Int64GetDatum(est_heap_wal_bytes);
	values[4] = Int64GetDatum(est_index_wal_bytes);
	values[5] = Int64GetDatum(n_dead_tup);
	values[6] = Int64GetDatum((int64) dirty_pages);
	values[7] = Int64GetDatum((int64) all_visible);
	values[8] = Int64GetDatum((int64) all_frozen);
