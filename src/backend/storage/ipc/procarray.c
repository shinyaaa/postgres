...
typedef struct XidHorizonBlockerCandidate
{
	...
	int			slot_index;		/* replication_slots[] index for slot
								 * blockers; -1 otherwise */
					  ...
					  int *nblockers)
{
	...
	for (int index = 0; index < arrayP->numProcs; index++)
	{
		...
		/*
		 * The caller itself never blocks its own horizon, and unlike the
		 * horizon computation we are not interested in its xmin; skip it.
		 * This exclusion is specific to this function; the two conditions
		 * below mirror ComputeXidHorizons().
		 */
		if (proc == MyProc)
			continue;
		 ...
		 * Skip over backends either vacuuming (which is ok with rows being
		 * removed, as long as pg_subtrans is not truncated) or doing logical
		 * decoding (which manages xmin separately, covered by the slot scan
		 * below).
		 */
		if (statusFlags & (PROC_IN_VACUUM | PROC_IN_LOGICAL_DECODING))
			continue;
		 ...
		 * the entry itself is filled in once afterwards.  Backends are
		 * provisionally recorded as "active"; the active vs.
		 * idle-in-transaction distinction is resolved from the reported
		 * backend state once all locks are released (see below), because the
		 * PGPROC alone cannot tell an idle-in-transaction session from one
		 * blocked on client I/O.
		 */
		...
		else if (TransactionIdEquals(proc_xmin, horizon))
		{
			...
			if (statusFlags & PROC_AFFECTS_ALL_HORIZONS)
				...
				type = XHB_XMIN_ACTIVE_TRANSACTION;
		}
		else
			continue;
	 ...
	 * effective xmin values, the same ones
	 * ReplicationSlotsComputeRequiredXmin() aggregates into the horizon
	 * (data.xmin/catalog_xmin can lag those, e.g. while a logical slot is
	 * being created).
	 */
	if (max_slots > 0)
	{
	 ...
	 * This is a bsearch over a process-local snapshot; matching on
	 * proc_number plus st_procpid guards against the PGPROC being reused
	 * since the scan.
	 */
					 ...
					 XidHorizonBlocker *blocker)
{
	...
	for (int i = 0; i < nblockers; i++)
	{
		if (best == NULL || blockers[i].type < best->type)
		{
			...
			/*
			 * xid-match types are the highest priority (the root cause
			 * holding the horizon), so nothing can outrank them; stop once we
			 * find one.
			 */
