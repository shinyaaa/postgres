...
SyncRepReleaseWaiters(void)
{
	 ...
	 *
	 * This does not require holding SyncRepLock: SyncRepGetSyncRecPtr() reads
	 * each walsender's position under that walsender's own spinlock, and the
	 * result is only used below either to take a lock-free fast path or, after
	 * acquiring SyncRepLock, to advance WalSndCtl->lsn[] under the protection
	 * of that lock.
	 */
	...
	/*
	 * Fast path: if none of the synced positions is ahead of what has already
	 * been advertised in shared memory, there is no waiter that we could
	 * release, so we can avoid acquiring SyncRepLock altogether.
	 *
	 * This is the common case for a standby whose reply does not move the
	 * aggregate release point, e.g. a nearby standby in a multi-standby setup
	 * where a more distant standby is the one bounding the release point.
	 * Such a standby replies frequently, and without this check every reply
	 * would take SyncRepLock exclusively only to find nothing to do, which
	 * contends with backends entering the queue in SyncRepWaitForLSN().
	 *
	 * The lsn[] values only ever move forward and are read atomically, so a
	 * stale (older) read can only make us acquire the lock and recheck below
	 * unnecessarily; it can never make us skip a wakeup that is due.  A value
	 * that is already >= our position means some other actor has advanced the
	 * release point that far, and hence has already woken (or is about to
	 * wake) every waiter we would have released.
	 */
	...
	/*
	 * At least one position has advanced, so acquire the lock and release the
	 * waiters that have been reached.
	 */
	LWLockAcquire(SyncRepLock, LW_EXCLUSIVE);
