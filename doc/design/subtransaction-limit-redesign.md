...
**csnlog エントリのロックフリー読み — 実装済み**。slru.c にスロット単位の変更カウンタ
（seqlock 方式）を追加した。カウンタはスロットの内容が保持ページと対応しなくなる遷移
（別ページへの claim・ゼロ初期化・無効化・read 失敗）でのみバンクロック下でインクリメント
され、書き出し（内容不変）ではされない。読者（`SimpleLruTryReadUInt64()`）はロックなしで
バンク内をスキャンし、「対象ページ・内容安定状態・カウンタ不変」を読みの前後で検証できた
場合のみ値を採用する。これにより evict→同一ページ再読込の ABA を含む全レースを検出し、
検証に失敗したらロック読みへフォールバックする。可視性ホットパスは通常 SLRU ロックを
一切取らなくなった。8 バイト single-copy atomicity のないプラットフォームでは
フォールバックのみにコンパイルされる。

**KnownAssignedXids の縮退 — 第一段階実装済み**。スタンバイの `GetSnapshotData()` は
KnownAssignedXids 全体のスナップショットへのコピーをやめ、最古エントリの xmin 取り込み
（先頭要素で停止する `KnownAssignedXidsGetOldestXmin()`）のみになった。スナップショット
取得コストが追跡中 XID 数に比例しなくなる。KAX 自体は xmin 水平線・リカバリコンフリクト・
リカバリ中の `TransactionIdIsInProgress()` のために残る。完全削除はスタンバイの水平線
追跡の再設計（独立課題）。

**`SubTransSetParent()` の遅延化 — 検討の結果、不採用を正式決定**。可視性パスからは
既に pg_subtrans を排除済みなので、残る動機は割り当て時の書き込みコスト削減のみ。
しかし残存読者を精査すると、`XactLockTableWait()`（行ロック競合時に待機対象のトップ
XID を得る）が**任意の書き込み済み subxid** に対して親情報を要求する。サブトランザク
ションの xact ロックはサブコミット時に解放されるため、親へ変換せずに subxid のロックを
待つと「サブコミット済み・親は実行中」の行に対して早すぎる返答をし、lmgr.c の
フォールバックは 1ms ポーリングに退化する。つまり親記録を省けるのは「ロック競合が
起こり得ない subxid」に限られ、それは事前に判定できない。書き込み自体はバンクロック下の
4 バイトストアで割り当てパスの支配的コストではないため、常時書き込みを維持する
（Simon Riggs の 2022 年提案が停滞したのと同じ帰結）。

**logical decoding の CSN 化 — 分析済み、独立課題として残置**。historic snapshot は
「コミット済み XID の列挙」という逆意味論で、専用の `HeapTupleSatisfiesHistoricMVCC()`
を使う。suboverflow の病理とは独立であり、本設計の目的（R1〜R3）には影響しない。
CSN 化するには過去の任意の WAL 位置のスナップショットを CSN で表現する必要があるが、
プライマリの CSN（公開順カウンタ）は WAL に記録されない。方針としては、スタンバイと
同じ構図で「SnapBuild がリプレイ順＝ローカル CSN を再構成する」方式が有望。
ReorderBufferCopySnap が subxip を別意味で使っている等、影響範囲が広いため独立
パッチシリーズとする。`SnapshotData` からの subxip 系フィールドの物理削除はこの後で
のみ可能（現状、フィールドは残るが本番ビルドでは可視性から完全に切り離されている）。

**ベンチマーク — 実施済み**（§8.3）。

### 8.3 ベンチマーク結果

環境: 4 vCPU コンテナ、非 assert ビルド、shared_buffers 512MB〜1GB、autovacuum off。
ベースライン＝本パッチ適用直前の master（同一環境・同一手順）。コンテナのため
pgbench 系は分散が大きく（±25%）、決定的マイクロベンチを主指標とする。

**A. suboverflow 病理の決定的再現**（2,000,000 行のテーブル全行を 100 個の
サブトランザクションで更新したトランザクションを open のまま保持し、他のコミットで
スナップショット xmax を subxid より先へ進めた状態で、別セッションが全行スキャン）:

| 指標 | ベースライン | パッチ適用後 |
|------|--------------:|--------------:|
| 単一読者の全行スキャン | 610〜634 ms | **322〜333 ms（約 1.9 倍）** |
| スキャンあたり pg_subtrans アクセス | 約 700 万回 | **0 回** |
| 4 並列読者が 20 秒間に完了したスキャン数 | 32 | **60（1.88 倍）** |

ベースラインのコストは全タプルの xmin/xmax についての
`SubTransGetTopmostTransaction()`（親チェーン分の SLRU 参照×バンクロック共有取得）。
パッチ適用後は 1 タプルあたりロックフリーの csnlog 読み 1 回になる。本測定は 4 コア・
subtrans ウィンドウが全部バッファに乗る「ベースラインに最も有利な」条件であり、
実障害（ウィンドウがバッファを超えディスク I/O + ロック収束が始まる規模、数十コアでの
ロック競合）ではベースライン側の劣化が桁で悪化する一方、パッチ側は共有ロックを
取らないため悪化要因がない。

**B. 非オーバーフロー時のオーバーヘッド確認**（回帰がないことの確認）:

| 指標 | ベースライン | パッチ適用後 |
|------|--------------:|--------------:|
| 未ヒント 2M 行スキャン（可視性判定＋ヒント設定） | 82〜87 ms | 81〜85 ms（同等） |
| ヒント済み 2M 行スキャン | 26〜32 ms | 26〜33 ms（同等） |
| pgbench tpcb-like 8cli（コミットパスに csnlog スタンプ追加） | 2,847〜3,157 tps | 2,763〜3,032 tps（分散内） |
| pgbench select-only 8cli | 112k〜146k tps | 80k〜147k tps（分散内） |

**C. 混合ワークロード**（長時間トランザクション＋80-subxid ループ＋tpcb 書き込み並走下で
25 秒間の select-only、scale 10）: プライマリ read 48.3k → 47.9k tps、スタンバイ read
47.8k → 46.1k tps（いずれも同等）。ただし read 中の pg_subtrans アクセスはプライマリ
623 万回 → 5.4 万回（残余は書き込みパスの `TransactionIdIsInProgress()`）、スタンバイ
65 万回 → 3.6 万回。この規模（XID ウィンドウ約 20 万）ではベースラインも SLRU バッファに
収まり TPS 崩壊に至らないため、TPS 差は A の条件で顕在化する。

### 8.4 残課題（更新後）

- KnownAssignedXids の完全削除（スタンバイ水平線追跡の CSN ベース再設計）
- logical decoding の CSN 化と `SnapshotData` からの subxip 系フィールド物理削除（§8.2）
- 大規模ハードウェア（数十コア・subtrans ウィンドウ > バッファ）での再計測
